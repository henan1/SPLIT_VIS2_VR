...
void  Trackball::mouseDown(int state, int x, int y)
{
	...
	if (behavior & ROTATE && state==0){//action_tab[_ROTATE] == state) {
		action = _ROTATE;
	}
...
void  Trackball::mouseMotion(int x, int y)
{
	...
	case _ROTATE:
	{
		...
		myVector3f end = trackballMapping(x, y);
		myVector3f axis = cross(start, end);
	...
	case _DOLLY:
	{
		Scalar dolly_z = (start_y - y) * dolly_scale;//cerr<<dolly_z<<endl;
		Matrix4f dolly_mat;
...
myVector3f Trackball::trackballMapping(int x, int y)
{
	myVector3f v;
	float d;
...
void Trackball::setEye(const myVector3f & e)
{  
	eye = e;  
}

void Trackball::setFocus(const myVector3f & f)
{  
	focus = f; 

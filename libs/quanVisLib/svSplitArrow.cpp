...
namespace __svl_lib {
svSplitArrow::svSplitArrow(svVectorField *f):svGlyph()//svVectorField* f, int numPlane)
{
  field = f;
  ...
  svGlyph::STILL_UPDATE = false;
    ...
    {		
		...
		for(int j=0;j<glyph[i].size();j++)
		{
			...
			svScalar coe = mag[i][i]/pow(10.,exp);
			...
			RenderCone(base, base*3,4);// glutSolidCone(base, base*3, 4, 1);
			glPopMatrix();

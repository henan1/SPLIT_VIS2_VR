...
int genVTKfromOrg::CheckFormat(char *file)
{
  ...
  if(infile.fail())
  {
	...
	cout<<"Load QDOT data failed. prog exit \n"<<endl;
...
void genVTKfromOrg::ReadRawData(char *file, int DEN_SCALAR)
{
  ...
  else
  {
	...
	cout << filename << " ";
	cout << "Load QDOT data failed. Wrong format. Prog exit\n" <<endl;
	exit(-1);
  }
...
cout<<"Split data by "<<numPlane<<" layers along direction ("<<dir[0]<<","<<dir[1]<<","<<dir[2]<<")"<<endl;
...
*/
    ...
    sprintf(str2, "%s/zlayer%d.txt", file, i);    

    ofstream layerout(str2);
    ...
    layerout<<splitData[i].size()<<endl; 


    //layerout<<rbbox[0]-lbbox[0]<<" "<<rbbox[1]-lbbox[1]<<" "<<plane_distance<<endl;
  ...
  //    outfile<<splitData[i][j].px<<" "<<splitData[i][j].py<<" "<<splitData[i][j].pz<<endl;
    ...
    //  layerout<<splitData[i][j].vx<<" "<<splitData[i][j].vy<<" "<<splitData[i][j].vz<<" ";
    }
    outfile<<endl; 
    ...
    for(int j=0;j<splitData[i].size();j++)
    {
     ...
     // layerout<<splitData[i][j].den<<endl;
    }
    outfile<<endl;
